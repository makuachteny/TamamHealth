  ...
  return (
    <div className="space-y-5">
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            <h2 className="text-lg font-bold" style={{ color: 'var(--text-primary)' }}>
              SBAR Handoff · {fullName}
            </h2>
            <p className="text-xs mt-0.5" style={{ color: 'var(--text-muted)' }}>
              Auto-generated from chart · {formatDateTime(new Date().toISOString())}
            </p>
          </div>
        </div>
      ...
      >
        ...
        {allergies.length === 0 ? (
          <span className="text-sm" style={{ color: 'var(--text-muted)' }}>None known</span>
        ) : (
      ...
      <Section letter="S" title="Situation" hint="Why am I telling you about this patient?">
        <p>
          <strong>{fullName}</strong>
          {age != null && <>, {age}-year-old {patient.gender.toLowerCase()}</>}
        ...
        {latestTriage ? (
          <p className="mt-2">
            ...
            ({formatDateTime(latestTriage.triagedAt)}). Status: {latestTriage.status}.
            {latestTriage.chiefComplaint && (
              <> Chief complaint: <em>{latestTriage.chiefComplaint}</em>.</>
            )}
          </p>
        ) : latestRecord?.chiefComplaint ? (
          <p className="mt-2">
            Last consult ({formatDateTime(latestRecord.consultedAt || latestRecord.visitDate)}):{' '}
            <em>{latestRecord.chiefComplaint}</em>
          </p>
        ) : (
          <p className="mt-2" style={{ color: 'var(--text-muted)' }}>No active situation recorded.</p>
        )}
      </Section>

      <Section letter="B" title="Background" hint="What is the relevant history?">
        <div className="data-row-divider-sm">
          ...
          {activeRx.length > 0 && (
            <div>
              <span className="text-xs font-semibold uppercase" style={{ color: 'var(--text-muted)', letterSpacing: '0.06em' }}>
                Current medications · {activeRx.length}
              </span>
      ...
      <Section letter="A" title="Assessment" hint="What do I think the problem is?">
        {latestVitals ? (
          <div>
            <div className="flex items-center gap-2 mb-3">
              ...
              <span className="text-xs font-semibold uppercase" style={{ color: 'var(--text-secondary)', letterSpacing: '0.06em' }}>
                Latest vitals · {formatDateTime(latestVitals.recordedAt)}
              </span>
            </div>
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-2">
              <Tile label="Temp" value={`${latestVitals.temperature}°C`} />
              <Tile label="BP" value={`${latestVitals.systolic}/${latestVitals.diastolic}`} />
              <Tile label="Pulse" value={`${latestVitals.pulse}`} />
              <Tile label="RR" value={`${latestVitals.respiratoryRate}`} />
              <Tile label="SpO₂" value={`${latestVitals.oxygenSaturation}%`} />
              <Tile label="Wt" value={`${latestVitals.weight} kg`} />
            </div>
          </div>
        ) : (
                ...
                <li key={l._id} className="flex items-baseline justify-between gap-3">
                  <span>
                    ...
                    <span className="ml-2 text-xs" style={{ color: 'var(--text-muted)' }}>
                      ref {l.referenceRange} · {formatDateTime(l.completedAt)}
                    </span>
                  </span>
      ...
      <Section letter="R" title="Recommendation" hint="What needs to happen next?">
        <ul className="data-row-divider-sm">
          {pendingLabs.length > 0 && (
            <li>
              <span className="text-xs font-semibold uppercase" style={{ color: 'var(--text-muted)', letterSpacing: '0.06em' }}>
                Pending labs to follow up
              </span>
          ...
          {activeRx.length > 0 && (
            <li>
              ...
              <span style={{ color: 'var(--text-muted)' }}>
                {activeRx.length} active prescription{activeRx.length === 1 ? '' : 's'}
              </span>
            </li>
          )}
          ...
          {pendingLabs.length === 0 && !latestRecord?.followUp?.date && activeRx.length === 0 && !latestRecord?.treatmentPlan && (
            <li style={{ color: 'var(--text-muted)' }}>
              No outstanding orders or follow-up actions documented.
            </li>
          )}
        </ul>
      </Section>
      ...
      <p className="text-[11px] italic px-1" style={{ color: 'var(--text-muted)' }}>
        Auto-generated from this patient&apos;s chart. Verify against source records before acting on any item.
      </p>
    </div>
  );
}
