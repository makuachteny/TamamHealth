  ...
  return (
    <>
      ...
      <main className="page-container page-enter">
                    ...
                    <td className="px-4 py-2.5">
                      <div className="flex items-center gap-2.5 min-w-0">
                        <span className="min-w-0">
                          ...
                          {p.triagePriority && (
                            <span className="inline-block w-2 h-2 rounded-full ml-1.5 align-middle" style={{
                              background: p.triagePriority === 'RED' ? 'var(--color-danger)' : p.triagePriority === 'YELLOW' ? 'var(--color-warning)' : 'var(--color-success)',
                            }} />
                          )}
                        </span>
                      </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-5 flex-shrink-0" style={{ order: 1 }}>
          ...
          <QuickActionsCard
            ...
            actions={[
              ...
              { label: t('action.newConsultation'), icon: ClipboardList, action: () => setSoapModalOpen(true), color: 'var(--accent-primary)' },
              { label: t('dashboard.quickPrescribe'), icon: Pill, action: () => setPrescribeModalOpen(true), color: '#0D9488' },
              { label: t('dashboard.quickLabOrder'), icon: TestTube, action: () => setLabModalOpen(true), color: '#7C3AED' },
              ...
              { label: t('dashboard.birthReg'), icon: Baby, action: () => router.push('/births'), color: 'var(--accent-primary)' },
        ...
        {/* ═══ DOCUMENTS TO SIGN — Chart Desktop signing inbox ═══ */}
        {(unsignedDrafts.length > 0 || awaitingCosign.length > 0 || heldAssessments.length > 0) && (
          <div className="dash-card overflow-hidden flex-shrink-0" style={{ order: 2 }}>
            <div className="px-5 py-3 border-b flex items-center justify-between" style={{ borderColor: 'var(--border-light)' }}>
              <h3 className="font-semibold text-sm inline-flex items-center gap-2">
                ...
                <span className="text-[11px] font-normal" style={{ color: 'var(--text-muted)' }}>
                  ({unsignedDrafts.length + awaitingCosign.length + heldAssessments.length})
                </span>
              </h3>
              <span className="text-[11px]" style={{ color: 'var(--text-muted)' }}>Notes awaiting your signature</span>
            </div>
            <div style={{ maxHeight: 264, overflowY: 'auto' }}>
              {heldAssessments.map((a) => (
                <div key={a._id} className="flex items-center gap-3 px-5 py-3" style={{ borderBottom: '1px solid var(--border-light)' }}>
                  <div className="flex-1 min-w-0">
                    <div className="text-[13px] font-semibold truncate" style={{ color: 'var(--text-primary)' }}>{signingPatientName(a.patientId)}</div>
                    <div className="text-[11px] truncate" style={{ color: 'var(--text-muted)' }}>
                      {a.instrumentName} · score {a.totalScore}{a.interpretation ? ` · ${a.interpretation}` : ''}
                    </div>
                  </div>
                  <span className="text-[9px] font-bold uppercase tracking-wider px-2 py-1 rounded flex-shrink-0" style={{ background: 'rgba(217,119,6,0.12)', color: '#B45309' }}>
                    Outcome measure
                  </span>
                  <button onClick={() => router.push(`/patients/${a.patientId}`)} className="btn btn-sm btn-secondary flex-shrink-0">
                    Review
                  </button>
                </div>
              ))}
              {awaitingCosign.map((r) => (
                <div key={r._id} className="flex items-center gap-3 px-5 py-3" style={{ borderBottom: '1px solid var(--border-light)' }}>
                  <div className="flex-1 min-w-0">
                    <div className="text-[13px] font-semibold truncate" style={{ color: 'var(--text-primary)' }}>{signingPatientName(r.patientId)}</div>
                    <div className="text-[11px] truncate" style={{ color: 'var(--text-muted)' }}>
                      {r.chiefComplaint} · signed by {r.signedByName || 'trainee'}
                    </div>
                  </div>
                  <span className="text-[9px] font-bold uppercase tracking-wider px-2 py-1 rounded flex-shrink-0" style={{ background: 'rgba(217,119,6,0.12)', color: 'var(--color-warning, #B45309)' }}>
                    {canCosign ? 'Needs co-sign' : 'Awaiting co-sign'}
                  </span>
                  <button onClick={() => router.push(`/patients/${r.patientId}`)} className="btn btn-sm btn-secondary flex-shrink-0">
                    Review
                  </button>
                </div>
              ))}
              {unsignedDrafts.map((r) => (
                <div key={r._id} className="flex items-center gap-3 px-5 py-3" style={{ borderBottom: '1px solid var(--border-light)' }}>
                  <div className="flex-1 min-w-0">
                    <div className="text-[13px] font-semibold truncate" style={{ color: 'var(--text-primary)' }}>{signingPatientName(r.patientId)}</div>
                    <div className="text-[11px] truncate" style={{ color: 'var(--text-muted)' }}>
                      {r.chiefComplaint} · {formatCompactDateTime(r.consultedAt || r.visitDate || r.createdAt)}
                    </div>
                  </div>
                  <span className="text-[9px] font-bold uppercase tracking-wider px-2 py-1 rounded flex-shrink-0" style={{ background: 'var(--overlay-subtle)', color: 'var(--text-muted)' }}>
                    Draft
                  </span>
                  <button onClick={() => router.push(`/patients/${r.patientId}`)} className="btn btn-sm btn-secondary flex-shrink-0">
                    Open
