...
export default function VitalStatisticsPage() {
  ...
  if (loading || !data) return <><TopBar title={t('vitalStats.topBarTitle')} /><main className="page-container flex items-center justify-center"><p className="text-sm" style={{ color: 'var(--text-muted)' }}>{t('vitalStats.loading')}</p></main></>;
  ...
  return (
    <>
      ...
      <main className="page-container page-enter">
        ...
        {/* Death Statistics */}
        <h2 className="font-semibold text-sm flex items-center gap-2 mb-3 mt-6"><Skull className="w-4 h-4" style={{ color: 'var(--color-danger)' }} /> {t('vitalStats.mortalityStatistics')}</h2>
        <div className="kpi-grid mb-6">
          {[
            { label: t('deaths.statTotalDeaths'), value: deathStats.total, icon: Skull, color: 'var(--color-danger)', bg: 'rgba(229,46,66,0.12)' },
