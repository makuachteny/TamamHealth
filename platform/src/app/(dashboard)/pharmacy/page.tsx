...
import { Pill, AlertTriangle, Loader2, Plus, X, Clock, Printer, Calendar, Package, ShoppingCart, User, ChevronRight, AlertOctagon } from '@/components/icons/lucide';
...
export default function PharmacyPage() {
  ...
  const filteredInventory = inventory.filter(i =>
    (stockFilter === 'all' || i.status === stockFilter) &&
    ...
    <>
      ...
      <main className="page-container page-enter">
                ...
                <tbody>
                  {filteredQueue.length === 0 ? (
                    ...
                    <tr key={rx._id} className="cursor-pointer hover:bg-[var(--table-row-hover)]" onClick={() => { if (rx.patientId) router.push(`/patients/${rx.patientId}`); }}>
                      <td><PatientName name={rx.patientName} nameClassName="text-sm font-medium" /></td>
          ...
          {activeTab === 'inventory' && (
            <>
              <FilterBar>
                <SearchInput value={search} onChange={setSearch} placeholder={t('pharmacy.searchMedicationsPlaceholder')} />
                <FilterSelect
                  value={stockFilter}
                  onChange={setStockFilter}
                  options={[
                    { value: 'all', label: 'All stock' },
                    { value: 'adequate', label: 'In stock' },
          ...
          {activeTab === 'patients' && (
            <>
              <FilterBar>
                <SearchInput value={search} onChange={(v) => { setSearch(v); setSelectedPatient(null); }} placeholder={t('pharmacy.searchPatientPlaceholder')} />
              </FilterBar>
              {activePatient ? (
                <div className="card-elevated overflow-hidden">
                  <div className="flex items-center justify-between px-4 pt-3 pb-2">
                    <div className="flex items-center gap-2">
                      <div className="w-8 h-8 rounded-full flex items-center justify-center text-[10px] font-bold text-white" style={{ background: 'var(--accent-primary)' }}>
                        {[activePatient.firstName, activePatient.surname].filter(Boolean).map(n => n[0]).join('')}
                      </div>
                      <div>
                      ...
                      {patientResults.map(p => {
                        ...
                        return (
                          <button key={p._id} onClick={() => setSelectedPatient(p._id)}
                            ...
                            <div className="w-9 h-9 rounded-full flex items-center justify-center text-[10px] font-bold text-white flex-shrink-0" style={{ background: 'var(--accent-primary)' }}>
                              {[p.firstName, p.surname].filter(Boolean).map(n => n[0]).join('')}
                            </div>
